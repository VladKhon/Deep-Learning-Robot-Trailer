...
tloc = t;                 
S0 = [0 0];
S = csvread('Datasets_Kinematics/New/PredictedSpeedsBest.csv');
S = vertcat(S0, S);
V = [t' S(:, 1)];
W = [t' S(:, 2)];
sim('AXON_Kinematics_CONF5.slx')
X_ref = csvread('Datasets_Kinematics/x.csv');
Y_ref = csvread('Datasets_Kinematics/y.csv');
x_ref = X_ref(:,end)
y_ref = Y_ref(:,end)
pause(1)
