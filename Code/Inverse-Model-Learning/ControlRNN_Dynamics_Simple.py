...
data_dir = 'Datasets_Dynamics/1env'
# X_train preparation
x_pos = np.array(pd.read_csv(os.path.join(data_dir,'x.csv')))
y_pos = np.array(pd.read_csv(os.path.join(data_dir,'y.csv')))
th1_head = np.array(pd.read_csv(os.path.join(data_dir,'th1.csv')))
th0_head = np.array(pd.read_csv(os.path.join(data_dir,'th0.csv')))
...
# Y_train preparation
tau1_tau2 = np.array(pd.read_csv(os.path.join(data_dir,'tau.csv')))
...
exponential_decay_fn = exponential_decay(lr0=0.01, s=500)

# # Callbacks
checkpoint_cb = keras.callbacks.ModelCheckpoint("Models/ControlRNN_Dynamics_Simple.h5", save_freq=20)
...
optimiser = keras.optimizers.Adam() # for later option setting
...
np.savetxt(os.path.join(data_dir, 'PredictedTorques.csv'), np.squeeze(Y_pred, axis=0), delimiter=",")
