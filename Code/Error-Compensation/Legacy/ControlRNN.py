...
model = keras.models.Sequential([keras.layers.LSTM(batch_input_shape=(None, None, dim_in), return_sequences=True, units=nb_units),
                                 keras.layers.LSTM(4, return_sequences=True),
                                 keras.layers.LSTM(4, return_sequences=True),
                                 keras.layers.Dense(dim_out) # keras.layers.TimeDistributed(keras.layers.Dense(dim_out))
                                 ])
